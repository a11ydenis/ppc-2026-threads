...
bool NalitovDDijkstrasAlgorithmOmp::PreProcessingImpl() {
  ...
  for (int v = 0; v < vn; ++v) {
    graph_[static_cast<std::size_t>(v)].resize(outdeg[static_cast<std::size_t>(v)]);
  }

  const auto vn_u = static_cast<std::size_t>(vn);
  std::unique_ptr<std::atomic<std::size_t>[]> row_next(new std::atomic<std::size_t>[vn_u]);
  for (std::size_t v = 0; v < vn_u; ++v) {
    row_next[v].store(0, std::memory_order_relaxed);
  }
...
#pragma omp parallel for default(none) schedule(guided) num_threads(omp_threads) shared(in, g, row_next, arc_count)
  for (std::ptrdiff_t ei = 0; ei < static_cast<std::ptrdiff_t>(arc_count); ++ei) {
    ...
    const std::size_t u = static_cast<std::size_t>(a.from);
